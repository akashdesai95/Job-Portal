...
$statement="UPDATE clients SET closejob=\"blocked\" WHERE id=?";
$query=$connection->prepare($statement);
if(!$query->execute(array($_GET['company_id']))){
	echo "error";
	die();
...
$statement="UPDATE postjob SET closejob=\"blocked\" WHERE company_id=?";
$query=$connection->prepare($statement);
if(!$query->execute(array($_GET['company_id']))){
	echo "error";
	die();
...
$statement="UPDATE jobapplications SET closejob=\"blocked\" WHERE company_id=?";
$query=$connection->prepare($statement);
if(!$query->execute(array($_GET['company_id']))){
	echo "error";
	die();
